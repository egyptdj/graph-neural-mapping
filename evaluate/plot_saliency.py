...
def main():
    ...
    normalized_array = plot_nii(saliency0subjects, roiimgaffine, os.path.join(opt.expdir, opt.savedir), 'gradcam_female_final')
    write_csv(normalized_array, roiimgarray, roimeta, opt.threshold, os.path.join(opt.expdir, opt.savedir), 'gradcam_female_final')
    normalized_array = plot_nii(saliency1subjects, roiimgaffine, os.path.join(opt.expdir, opt.savedir), 'gradcam_male_final')
    write_csv(normalized_array, roiimgarray, roimeta, opt.threshold, os.path.join(opt.expdir, opt.savedir), 'gradcam_male_final')
    normalized_array = plot_nii(saliency0earlysubjects, roiimgaffine, os.path.join(opt.expdir, opt.savedir), 'gradcam_female_early')
    write_csv(normalized_array, roiimgarray, roimeta, opt.threshold, os.path.join(opt.expdir, opt.savedir), 'gradcam_female_early')
    normalized_array = plot_nii(saliency1earlysubjects, roiimgaffine, os.path.join(opt.expdir, opt.savedir), 'gradcam_male_early')
    write_csv(normalized_array, roiimgarray, roimeta, opt.threshold, os.path.join(opt.expdir, opt.savedir), 'gradcam_male_early')

    normalized_array = plot_nii([subject for idx, subject in enumerate(saliency0subjects) if labels[idx]==0], roiimgaffine, os.path.join(opt.expdir, opt.savedir), 'gradcam_female_femalesubj_final')
    write_csv(normalized_array, roiimgarray, roimeta, opt.threshold, os.path.join(opt.expdir, opt.savedir), 'gradcam_female_femalesubj_final')
    normalized_array = plot_nii([subject for idx, subject in enumerate(saliency0subjects) if labels[idx]==1], roiimgaffine, os.path.join(opt.expdir, opt.savedir), 'gradcam_female_malesubj_final')
    write_csv(normalized_array, roiimgarray, roimeta, opt.threshold, os.path.join(opt.expdir, opt.savedir), 'gradcam_female_malesubj_final')
    normalized_array = plot_nii([subject for idx, subject in enumerate(saliency1subjects) if labels[idx]==0], roiimgaffine, os.path.join(opt.expdir, opt.savedir), 'gradcam_male_femalesubj_final')
    write_csv(normalized_array, roiimgarray, roimeta, opt.threshold, os.path.join(opt.expdir, opt.savedir), 'gradcam_male_femalesubj_final')
    normalized_array = plot_nii([subject for idx, subject in enumerate(saliency1subjects) if labels[idx]==1], roiimgaffine, os.path.join(opt.expdir, opt.savedir), 'gradcam_male_malesubj_final')
    write_csv(normalized_array, roiimgarray, roimeta, opt.threshold, os.path.join(opt.expdir, opt.savedir), 'gradcam_male_malesubj_final')
    normalized_array = plot_nii([subject for idx, subject in enumerate(saliency0earlysubjects) if labels[idx]==0], roiimgaffine, os.path.join(opt.expdir, opt.savedir), 'gradcam_female_femalesubj_early')
    write_csv(normalized_array, roiimgarray, roimeta, opt.threshold, os.path.join(opt.expdir, opt.savedir), 'gradcam_female_femalesubj_early')
    normalized_array = plot_nii([subject for idx, subject in enumerate(saliency0earlysubjects) if labels[idx]==1], roiimgaffine, os.path.join(opt.expdir, opt.savedir), 'gradcam_female_malesubj_early')
    write_csv(normalized_array, roiimgarray, roimeta, opt.threshold, os.path.join(opt.expdir, opt.savedir), 'gradcam_female_malesubj_early')
    normalized_array = plot_nii([subject for idx, subject in enumerate(saliency1earlysubjects) if labels[idx]==0], roiimgaffine, os.path.join(opt.expdir, opt.savedir), 'gradcam_male_femalesubj_early')
    write_csv(normalized_array, roiimgarray, roimeta, opt.threshold, os.path.join(opt.expdir, opt.savedir), 'gradcam_male_femalesubj_early')
    normalized_array = plot_nii([subject for idx, subject in enumerate(saliency1earlysubjects) if labels[idx]==1], roiimgaffine, os.path.join(opt.expdir, opt.savedir), 'gradcam_male_malesubj_early')
    write_csv(normalized_array, roiimgarray, roimeta, opt.threshold, os.path.join(opt.expdir, opt.savedir), 'gradcam_male_malesubj_early')


    # # plot backprop gradcam based saliency
    # saliency0 = []
    # saliency1 = []
    # saliency0_early = []
    # saliency1_early = []
    # saliency0_diag = []
    # saliency1_diag = []
    # saliency0_early_diag = []
    # saliency1_early_diag = []
    # labels = []
    #
    # for current_fold in opt.fold_idx:
    #     saliency0_diag.append(np.load(os.path.join(opt.expdir, 'saliency', str(current_fold), 'grad_saliency_female.npy')))
    #     saliency1_diag.append(np.load(os.path.join(opt.expdir, 'saliency', str(current_fold), 'grad_saliency_male.npy')))
    #     saliency0_early_diag.append(np.load(os.path.join(opt.expdir, 'saliency', str(current_fold), 'grad_saliency_female_early.npy')))
    #     saliency1_early_diag.append(np.load(os.path.join(opt.expdir, 'saliency', str(current_fold), 'grad_saliency_male_early.npy')))
    #     labels.append(np.load(os.path.join(opt.expdir, 'latent', str(current_fold), 'labels.npy')))
    #     saliency0.append(np.load(os.path.join(opt.expdir, 'saliency', str(current_fold), 'gradcam_saliency_female.npy')))
    #     saliency1.append(np.load(os.path.join(opt.expdir, 'saliency', str(current_fold), 'gradcam_saliency_male.npy')))
    #     saliency0_early.append(np.load(os.path.join(opt.expdir, 'saliency', str(current_fold), 'gradcam_saliency_female_early.npy')))
    #     saliency1_early.append(np.load(os.path.join(opt.expdir, 'saliency', str(current_fold), 'gradcam_saliency_male_early.npy')))
    #
    # saliency0_diag = np.diagonal(np.concatenate(saliency0_diag, 0), axis1=1, axis2=2)
    # saliency1_diag = np.diagonal(np.concatenate(saliency1_diag, 0), axis1=1, axis2=2)
    # saliency0early_diag = np.diagonal(np.concatenate(saliency0_early_diag, 0), axis1=1, axis2=2)
    # saliency1early_diag = np.diagonal(np.concatenate(saliency1_early_diag, 0), axis1=1, axis2=2)
    #
    # saliency0_diag = saliency0_diag / np.amax(np.abs(saliency0_diag), axis=0)
    # saliency1_diag = saliency1_diag / np.amax(np.abs(saliency1_diag), axis=0)
    # saliency0early_diag = saliency0early_diag / np.amax(np.abs(saliency0early_diag), axis=0)
    # saliency1early_diag = saliency1early_diag / np.amax(np.abs(saliency1early_diag), axis=0)
    #
    # saliency0 = np.concatenate(saliency0, 0)
    # saliency1 = np.concatenate(saliency1, 0)
    # saliency0early = np.concatenate(saliency0_early, 0)
    # saliency1early = np.concatenate(saliency1_early, 0)
    #
    # saliency0 *= saliency0_diag
    # saliency1 *= saliency1_diag
    # saliency0early *= saliency0early_diag
    # saliency1early *= saliency1early_diag
    #
    # del saliency0_diag
    # del saliency1_diag
    # del saliency0early_diag
    # del saliency1early_diag
    #
    # labels = np.concatenate(labels, 0).squeeze()
    # female_index = np.where(labels==0)
    # male_index = np.where(labels==1)
    ...
    #     saliency0earlyarray = roiimgarray.copy()
    #     saliency1earlyarray = roiimgarray.copy()
    #     print("EXTRACTING BACKPROP-GRADCAM-SALIENCY SUBJECT: {} (LABEL:{})".format(subjidx, labels[subjidx]))
    #     for i, (s0, s1, s0e, s1e) in enumerate(zip(sal0, sal1, sal0early, sal1early)):
    #         roi_voxel_idx = np.where(roiimgarray==i+1)
    #         for j in range(roi_voxel_idx[0].shape[0]):
    #             saliency0array[roi_voxel_idx[0][j], roi_voxel_idx[1][j], roi_voxel_idx[2][j]] = s0
    ...
    #     saliency1earlysubjects.append(saliency1earlyarray)
    #
    # normalized_array = plot_nii(saliency0subjects, roiimgaffine, os.path.join(opt.expdir, opt.savedir), 'backprop_gradcam_female_final')
    # write_csv(normalized_array, roiimgarray, roimeta, opt.threshold, os.path.join(opt.expdir, opt.savedir), 'backprop_gradcam_female_final')
    # normalized_array = plot_nii(saliency1subjects, roiimgaffine, os.path.join(opt.expdir, opt.savedir), 'backprop_gradcam_male_final')
    # write_csv(normalized_array, roiimgarray, roimeta, opt.threshold, os.path.join(opt.expdir, opt.savedir), 'backprop_gradcam_male_final')
    # normalized_array = plot_nii(saliency0earlysubjects, roiimgaffine, os.path.join(opt.expdir, opt.savedir), 'backprop_gradcam_female_early')
    # write_csv(normalized_array, roiimgarray, roimeta, opt.threshold, os.path.join(opt.expdir, opt.savedir), 'backprop_gradcam_female_early')
    # normalized_array = plot_nii(saliency1earlysubjects, roiimgaffine, os.path.join(opt.expdir, opt.savedir), 'backprop_gradcam_male_early')
    # write_csv(normalized_array, roiimgarray, roimeta, opt.threshold, os.path.join(opt.expdir, opt.savedir), 'backprop_gradcam_male_early')
    #
    # normalized_array = plot_nii([subject for idx, subject in enumerate(saliency0subjects) if labels[idx]==0], roiimgaffine, os.path.join(opt.expdir, opt.savedir), 'backprop_gradcam_female_femalesubj_final')
    # write_csv(normalized_array, roiimgarray, roimeta, opt.threshold, os.path.join(opt.expdir, opt.savedir), 'backprop_gradcam_female_femalesubj_final')
    # normalized_array = plot_nii([subject for idx, subject in enumerate(saliency0subjects) if labels[idx]==1], roiimgaffine, os.path.join(opt.expdir, opt.savedir), 'backprop_gradcam_female_malesubj_final')
    # write_csv(normalized_array, roiimgarray, roimeta, opt.threshold, os.path.join(opt.expdir, opt.savedir), 'backprop_gradcam_female_malesubj_final')
    # normalized_array = plot_nii([subject for idx, subject in enumerate(saliency1subjects) if labels[idx]==0], roiimgaffine, os.path.join(opt.expdir, opt.savedir), 'backprop_gradcam_male_femalesubj_final')
    # write_csv(normalized_array, roiimgarray, roimeta, opt.threshold, os.path.join(opt.expdir, opt.savedir), 'backprop_gradcam_male_femalesubj_final')
    # normalized_array = plot_nii([subject for idx, subject in enumerate(saliency1subjects) if labels[idx]==1], roiimgaffine, os.path.join(opt.expdir, opt.savedir), 'backprop_gradcam_male_malesubj_final')
    # write_csv(normalized_array, roiimgarray, roimeta, opt.threshold, os.path.join(opt.expdir, opt.savedir), 'backprop_gradcam_male_malesubj_final')
    # normalized_array = plot_nii([subject for idx, subject in enumerate(saliency0earlysubjects) if labels[idx]==0], roiimgaffine, os.path.join(opt.expdir, opt.savedir), 'backprop_gradcam_female_femalesubj_early')
    # write_csv(normalized_array, roiimgarray, roimeta, opt.threshold, os.path.join(opt.expdir, opt.savedir), 'backprop_gradcam_female_femalesubj_early')
    # normalized_array = plot_nii([subject for idx, subject in enumerate(saliency0earlysubjects) if labels[idx]==1], roiimgaffine, os.path.join(opt.expdir, opt.savedir), 'backprop_gradcam_female_malesubj_early')
    # write_csv(normalized_array, roiimgarray, roimeta, opt.threshold, os.path.join(opt.expdir, opt.savedir), 'backprop_gradcam_female_malesubj_early')
    # normalized_array = plot_nii([subject for idx, subject in enumerate(saliency1earlysubjects) if labels[idx]==0], roiimgaffine, os.path.join(opt.expdir, opt.savedir), 'backprop_gradcam_male_femalesubj_early')
    # write_csv(normalized_array, roiimgarray, roimeta, opt.threshold, os.path.join(opt.expdir, opt.savedir), 'backprop_gradcam_male_femalesubj_early')
    # normalized_array = plot_nii([subject for idx, subject in enumerate(saliency1earlysubjects) if labels[idx]==1], roiimgaffine, os.path.join(opt.expdir, opt.savedir), 'backprop_gradcam_male_malesubj_early')
    # write_csv(normalized_array, roiimgarray, roimeta, opt.threshold, os.path.join(opt.expdir, opt.savedir), 'backprop_gradcam_male_malesubj_early')

    # # plot saturated backprop gradcam based saliency
    # saliency0 = []
    # saliency1 = []
    # saliency0_early = []
    # saliency1_early = []
    ...
    #     saliency0earlyarray = roiimgarray.copy()
    #     saliency1earlyarray = roiimgarray.copy()
    #     print("EXTRACTING SATURATED BACKPROP-GRADCAM-SALIENCY SUBJECT: {} (LABEL:{})".format(subjidx, labels[subjidx]))
    #     for i, (s0, s1, s0e, s1e) in enumerate(zip(sal0, sal1, sal0early, sal1early)):
    #         roi_voxel_idx = np.where(roiimgarray==i+1)
    #         for j in range(roi_voxel_idx[0].shape[0]):
    #             saliency0array[roi_voxel_idx[0][j], roi_voxel_idx[1][j], roi_voxel_idx[2][j]] = s0
    ...
    #     saliency1earlysubjects.append(saliency1earlyarray)
    #
    # normalized_array = plot_nii(saliency0subjects, roiimgaffine, os.path.join(opt.expdir, opt.savedir), 'saturated_backprop_gradcam_female_final')
    # write_csv(normalized_array, roiimgarray, roimeta, opt.threshold, os.path.join(opt.expdir, opt.savedir), 'saturated_backprop_gradcam_female_final')
    # normalized_array = plot_nii(saliency1subjects, roiimgaffine, os.path.join(opt.expdir, opt.savedir), 'saturated_backprop_gradcam_male_final')
    # write_csv(normalized_array, roiimgarray, roimeta, opt.threshold, os.path.join(opt.expdir, opt.savedir), 'saturated_backprop_gradcam_male_final')
    # normalized_array = plot_nii(saliency0earlysubjects, roiimgaffine, os.path.join(opt.expdir, opt.savedir), 'saturated_backprop_gradcam_female_early')
    # write_csv(normalized_array, roiimgarray, roimeta, opt.threshold, os.path.join(opt.expdir, opt.savedir), 'saturated_backprop_gradcam_female_early')
    # normalized_array = plot_nii(saliency1earlysubjects, roiimgaffine, os.path.join(opt.expdir, opt.savedir), 'saturated_backprop_gradcam_male_early')
    # write_csv(normalized_array, roiimgarray, roimeta, opt.threshold, os.path.join(opt.expdir, opt.savedir), 'saturated_backprop_gradcam_male_early')
    #
    # normalized_array = plot_nii([subject for idx, subject in enumerate(saliency0subjects) if labels[idx]==0], roiimgaffine, os.path.join(opt.expdir, opt.savedir), 'saturated_backprop_gradcam_female_femalesubj_final')
    # write_csv(normalized_array, roiimgarray, roimeta, opt.threshold, os.path.join(opt.expdir, opt.savedir), 'saturated_backprop_gradcam_female_femalesubj_final')
    # normalized_array = plot_nii([subject for idx, subject in enumerate(saliency0subjects) if labels[idx]==1], roiimgaffine, os.path.join(opt.expdir, opt.savedir), 'saturated_backprop_gradcam_female_malesubj_final')
    # write_csv(normalized_array, roiimgarray, roimeta, opt.threshold, os.path.join(opt.expdir, opt.savedir), 'saturated_backprop_gradcam_female_malesubj_final')
    # normalized_array = plot_nii([subject for idx, subject in enumerate(saliency1subjects) if labels[idx]==0], roiimgaffine, os.path.join(opt.expdir, opt.savedir), 'saturated_backprop_gradcam_male_femalesubj_final')
    # write_csv(normalized_array, roiimgarray, roimeta, opt.threshold, os.path.join(opt.expdir, opt.savedir), 'saturated_backprop_gradcam_male_femalesubj_final')
    # normalized_array = plot_nii([subject for idx, subject in enumerate(saliency1subjects) if labels[idx]==1], roiimgaffine, os.path.join(opt.expdir, opt.savedir), 'saturated_backprop_gradcam_male_malesubj_final')
    # write_csv(normalized_array, roiimgarray, roimeta, opt.threshold, os.path.join(opt.expdir, opt.savedir), 'saturated_backprop_gradcam_male_malesubj_final')
    # normalized_array = plot_nii([subject for idx, subject in enumerate(saliency0earlysubjects) if labels[idx]==0], roiimgaffine, os.path.join(opt.expdir, opt.savedir), 'saturated_backprop_gradcam_female_femalesubj_early')
    # write_csv(normalized_array, roiimgarray, roimeta, opt.threshold, os.path.join(opt.expdir, opt.savedir), 'saturated_backprop_gradcam_female_femalesubj_early')
    # normalized_array = plot_nii([subject for idx, subject in enumerate(saliency0earlysubjects) if labels[idx]==1], roiimgaffine, os.path.join(opt.expdir, opt.savedir), 'saturated_backprop_gradcam_female_malesubj_early')
    # write_csv(normalized_array, roiimgarray, roimeta, opt.threshold, os.path.join(opt.expdir, opt.savedir), 'saturated_backprop_gradcam_female_malesubj_early')
    # normalized_array = plot_nii([subject for idx, subject in enumerate(saliency1earlysubjects) if labels[idx]==0], roiimgaffine, os.path.join(opt.expdir, opt.savedir), 'saturated_backprop_gradcam_male_femalesubj_early')
    # write_csv(normalized_array, roiimgarray, roimeta, opt.threshold, os.path.join(opt.expdir, opt.savedir), 'saturated_backprop_gradcam_male_femalesubj_early')
    # normalized_array = plot_nii([subject for idx, subject in enumerate(saliency1earlysubjects) if labels[idx]==1], roiimgaffine, os.path.join(opt.expdir, opt.savedir), 'saturated_backprop_gradcam_male_malesubj_early')
    # write_csv(normalized_array, roiimgarray, roimeta, opt.threshold, os.path.join(opt.expdir, opt.savedir), 'saturated_backprop_gradcam_male_malesubj_early')
    #
